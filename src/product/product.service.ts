...
export const get = async ({
  ...
  const technicalSpecificationsElement = document.querySelector(
    "table#productDetails_techSpec_section_1, #productDetails_techSpec_section_1 table, #productDetails_techSpec_section_1"
  ) as HTMLTableElement | null;
  const technicalSpecifications = (
    technicalSpecificationsElement && technicalSpecificationsElement.tagName === 'TABLE'
  ...
  // Fallback for new Amazon list-based detail bullets
  if (additionalInformations.length === 0) {
    const detailBulletsDiv = document.querySelector("#detailBullets_feature_div");
    if (detailBulletsDiv) {
      const listItems = detailBulletsDiv.querySelectorAll("ul > li > span.a-list-item");
      listItems.forEach(li => {
        const textSpan = li.querySelector("span.a-text-bold");
        const valueSpan = li.querySelectorAll("span")[1]; // The span after the bold one

        if (textSpan && valueSpan) {
          const key = textSpan.textContent?.replace(/[\n\u200e\u200f\:]/g, "")?.trim();
          const value = valueSpan.textContent?.trim();
          if (key && value) {
            additionalInformations.push({ key, value });
          }
        }
      });
    }
  ...
  if (reviewElement) {
    ...
    const starRatingText = document.querySelector(
      'span[data-hook="rating-out-of-text"], i[data-hook="average-star-rating"] .a-icon-alt'
    )?.textContent;
    if (starRatingText) {
  ...
  const aiSummaryElement = document.querySelector("#cm-cr-dp-summarization-summary .a-size-base, #cm-cr-dp-summarization-summary");

  if (aiSummaryElement) {
    ratingReview.insights.overallResponseFromCustomer = aiSummaryElement.textContent?.trim() || "";
  ...
  const reviewsElement = document.querySelectorAll(
    "#cm-cr-dp-review-list div[data-hook='review'], div[data-hook='review']"
  );
  reviewsElement.forEach((element) => {
    try {
      ...
      ).filter(span => span.textContent?.trim() !== "").pop();
      const title = titleElement ? titleElement.textContent?.trim() : null;
